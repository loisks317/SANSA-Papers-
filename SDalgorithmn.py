...
medNeSD15=np.zeros(len(AllTimesSD[0]))
medNeSD16=np.zeros(len(AllTimesSD[1]))
medNeSD17=np.zeros(len(AllTimesSD[2]))
medNeSD18=np.zeros(len(AllTimesSD[3]))
medNe=[medNeSD15,medNeSD16,medNeSD17,medNeSD18]
labels=['15', '16', '17', '18']
for iFreq in range(len(medNe)):
    #
    temp=np.swapaxes(AllneSD[iFreq],1,0)[25:35] # gates 25 45
    tempMed=np.nanmedian(temp, axis=0)
    #
    # now get the closest eiscat value
    # first get nearest time
    NearestNeEIS=np.ones(len(AllTimesSD[iFreq]))*np.nan
    NearestNeSD=np.ones(len(AllTimesSD[iFreq]))*np.nan
    Neartesth=np.ones(len(AllTimesSD[iFreq]))*np.nan
    NearestHeat=np.ones(len(AllTimesSD[iFreq]))*np.nan
    Neartesth=np.ones(len(AllTimesSD[iFreq]))*np.nan
    upperBound=np.ones(len(AllTimesSD[iFreq]))*np.nan
    lowerBound=np.ones(len(AllTimesSD[iFreq]))*np.nan
    upperBoundH=np.ones(len(AllTimesSD[iFreq]))*np.nan
    lowerBoundH=np.ones(len(AllTimesSD[iFreq]))*np.nan
    #
    # heater density
    heaterDens=(((6*2*np.pi*1e6)**2) * (8.85*1e-12) * (9.11*1e-31))/(1.6*1e-19)**2
    for iTime in range(len(AllTimesSD[iFreq])):
        nearestETimeIndex=nearestDate(Times,AllTimesSD[iFreq][iTime])
        #
        # array choices at time index
        EISCATalts=np.swapaxes(alt,1,0)[nearestETimeIndex]
        EISCATalts=EISCATalts[13:22] # ones in our proper ray tracing frame
        EISCATnes=np.swapaxes(ne,1,0)[nearestETimeIndex][13:22]
        #
        # find nearest ne
        
        
        try:
            indexSD=  np.nanargmin(np.abs(EISCATnes-tempMed[iTime]))
            indexHeater=np.nanargmin(np.abs(EISCATnes-heaterDens))
            
            # get the highest point within 10%
            #uBound=1.05*tempMed[iTime]
            #lBound=.95*tempMed[iTime]
            Neartesth[iTime]=EISCATalts[indexSD]
            NearestHeat[iTime]=EISCATalts[indexHeater]
            #upperBound, upperBoundH=searchUpper(EISCATnes, uBound, EISCATalts, upperBound, upperBoundH, iTime, indexSD)
            #lowerBound, lowerBoundH=searchLower(EISCATnes, lBound, EISCATalts, lowerBound, lowerBoundH, iTime)
    ...
    # overplot
    #    
    time2=dt.date2num(AllTimesSD[iFreq])
    ...
    #
    # remove the nans
    concern=np.isnan(Neartesth)
    Neartesth=Neartesth[~concern]
    time2=time2[~concern]
    NearestNeSD=np.abs(NearestNeSD[~concern])
    NearestHeat=np.abs(NearestHeat[~concern])
    pdTime=pd.DatetimeIndex(dt.num2date(time2))
    df1=pd.DataFrame({'Eiscat':Neartesth}, index=pdTime)
    df2=pd.DataFrame({'SD':NearestNeSD}, index=pdTime)
    df3=pd.DataFrame({'Heater':NearestHeat}, index=pdTime)
    newy=df1['Eiscat'].resample('2T', how='median')
    newc=np.array(df2['SD'].resample('2T', how='median'))
    newh=np.array(df3['Heater'].resample('2T',how='median'))
    
    newx=dt.date2num(newy.index.to_pydatetime())
    newy=np.array(newy)
    newy2=np.array(newh)
    cc=np.ones(len(newy2))*heaterDens

    col2=ax.scatter(newx, newy,c=newc, s=80, cmap= 'viridis', vmin=vmin, vmax=vmax, edgecolor='silver', lw=2)
   ...
   # col2=ax.scatter(newx, newy2,c=cc, s=40, cmap= 'viridis', vmin=vmin, vmax=vmax, edgecolor='red', lw=2)
    ...
    plt.savefig('freq='+str(labels[iFreq])+'_20150312_EISCATSD_STEREO_CompareSpect.png')
    os.chdir('..')
    plt.close()
